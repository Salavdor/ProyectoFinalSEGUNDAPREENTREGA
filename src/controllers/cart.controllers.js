...
export default class ProductController extends Controllers {
  ...
  createCart = async (req, res, next) => {
    try {
      ...
      if (!newCart)
        return httpResponse.NotFound(res, "Algo salió mal al crear el carrito");
  ...
  getCartById = async (req, res, next) => {
    try {
      ...
      if (!result) return httpResponse.NotFound(res, "Carrito no obtenido");
  ...
  addProduct = async (req, res, next) => {
    try {
      ...
      if (!user) return httpResponse.NotFound(res, "usuario no obtenido");

      //Buscar carrito
      const cartID = user.cartID;
      let cart = await cartService.getCartById(cartID);
      if (!cart) return httpResponse.NotFound(res, "El carrito con el id proporcionado no existe");

      //Buscar producto
      let { pid } = req.params;
      let product = await productService.getProductById(pid);
      if (!product) return httpResponse.NotFound(res, "El producto con el id proporcionado no existe");
      ...
      if (!foundProductInCart) return httpResponse.NotFound(res, `producto: ${product.name} no existe en  ${cart._id} Cart`);
      ...
      if (!cart) return httpResponse.NotFound(res, "no se pudo guardar el producto");
  ...
  deleteOneProduct = async (req, res, next) => {
    try {
      ...
      const { userId } = req.user;
      const user = await userService.getById(userId);
      if (!user) return httpResponse.NotFound(res, "no se pudo obtener datos de usuario");
      ...
      if (!foundProductInCart) return httpResponse.NotFound(res, "El producto no existe en el carrito.");
      ...
      //Buscar cart nuevamente para el populate
      cart = await cartService.getCartById(cartID);
      if (!cart) return httpResponse.NotFound(res, "no se pudo borrar el producto");
      else return httpResponse.Ok(res, cart);
    } catch (error) {
        logger.error(error);
    }
  ...
  deleteAllProducts = async (req, res, next) => {
    try {
      ...
      if (!result) return httpResponse.NotFound(res, "Error al vaciar el carrito.");
      else return httpResponse.Ok(res, result);
    } catch (error) {
        logger.error(error);
    }
