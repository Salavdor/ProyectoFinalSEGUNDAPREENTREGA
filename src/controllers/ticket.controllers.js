...
export default class TicketController extends Controllers {
  ...
  getTicket = async (req, res) => {
    try {
      ...
      if (!result) return httpResponse.NotFound(res, "Error al obtener los tickets"), logger.warn(`🚫 Error al obtener los tickets`);
      else return httpResponse.Ok(res, result), logger.info(`Obteniendo tickets 👀`);
    } catch (error) {
      logger.error(error);
    }
  };
  ...
  getTicketById = async (req, res) => {
    try {
      ...
      if (!ticket) return httpResponse.NotFound(res, "Error al obtener ticket por su ID"), logger.warn(`🚫 Error al obtener ticket por su ID`);
      else return httpResponse.Ok(res, ticket), logger.info(`Obteniendo ticket por id 👀`);
    } catch (error) {
    logger.error(error);
    }
  };
  ...
  createTicket = async (user, productosConfirmados, cart, req, res) => {
    try {
      ...
      if (!result) return httpResponse.NotFound(res, "El usuario con el id proporcionado no existe"), logger.warn(`🚫 El usuario con el id ${user._id} proporcionado no existe`);
      ...
      if (!sum) return httpResponse.NotFound(res, "no se pudo realizar la suma de los productos"), logger.warn(`🚫 no se pudo realizar la suma de los productos`);
      ...
      else return httpResponse.Ok(res, ticketResult, cart.products_list), logger.info(`✅ ticket creado`);
    } catch (error) {
        logger.error(error);
    }
  };
