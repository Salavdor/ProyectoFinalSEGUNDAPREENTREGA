...
export default class TicketController extends Controllers {
  ...
  createTicket = async (user, productosConfirmados, cart, req, res) => {
    try {
      ...
      if (!resultUser) return httpResponse.NotFound(res, "El usuario con el id proporcionado no existe"), logger.warn(`🚫 El usuario con el id ${user._id} proporcionado no existe`);
