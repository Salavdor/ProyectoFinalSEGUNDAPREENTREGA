...
createProducts = async(req, res, next) => {
  try {
      ...
      if (!title || !categoria || !precio || !stock || !imagen) return httpResponse.NotFound(res, "Faltan parámetros para crear el producto."), logger.warn(`🚫 Faltan parámetros para crear el producto.`);
...
deleteProduct = async (req, res, next) => {
  try {
      ...
      // Obtener token de cookie
      const token = req.cookies.token
      if (!token) return httpResponse.NotFound(res, "No token provided"), logger.warn(`🚫 No token provided`);
      ...
      if (resultUser.role === "admin") {
          ...
          return httpResponse.NotFound(res, "No puede eliminar producto, no tienes permisos de administrador."), logger.warn(`🚫 No puede eliminar producto, no tienes permisos de administrador.`);
      }
